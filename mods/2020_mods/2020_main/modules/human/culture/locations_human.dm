/decl/background_detail/location/human
	name = "Mankind"
	description = ""
	capital = "Earth"
	hidden
	economic_power = 0.9

/decl/background_detail/location/human/earth/usa
	name = "United States of America"
	description = ""
	capital = "Washington, D.C"
	economic_power = 1.4

/decl/background_detail/location/human/earth/sov
	name = "Soviet Union"
	description = ""
	capital = "Moscow"
	economic_power = 1.2

/decl/background_detail/location/human/earth/uk
	name = "United Kingdom"
	description = ""
	capital = "London"
	economic_power = 1

/decl/background_detail/location/human/earth/ger
	name = "Germany - Earth"
	description = ""
	capital = "Berlin"
	economic_power = 1.3

/decl/background_detail/location/human/earth/frc
	name = "France - Earth"
	description = ""
	capital = "Paris"
	economic_power = 1

/decl/background_detail/location/human/earth/eu
	name = "Europe Union"
	description = ""
	capital = "Brussels"
	economic_power = 1

/decl/background_detail/location/human/earth/chn
	name = "China - Earth"
	description = ""
	capital = "Beijing"
	economic_power = 0.8

/decl/background_detail/location/human/earth/jap
	name = "Japan"
	description = ""
	capital = "Tokyo"
	economic_power = 1.2

/decl/background_detail/location/human/luna
	name = "Luna"
	ruling_body = "Private Sector"
	description = ""
	economic_power = 1

/decl/background_detail/location/human/mars
	name = "Mars"
	description = ""
	ruling_body = "Private Sector"
	economic_power = 1

/decl/background_detail/location/human/ceres
	name = "Ceres"
	description = ""
	ruling_body = "Private Sector"
	economic_power = 0.8

/decl/background_detail/location/human/spacer
	name = "Inner Space"
	description = ""
	ruling_body = "Private Sector"
	economic_power = 0.8
