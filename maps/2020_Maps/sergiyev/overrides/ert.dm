...
/decl/special_role/ert/Initialize()
	. = ..()
	leader_welcome_text = "As leader of the Emergency Response Team, you answer only to [global.using_map.company_name], and have authority to override the Captain where it is necessary to achieve your mission goals. It is recommended that you attempt to cooperate with the Commanding Officer where possible, however."
...
/decl/special_role/ert/greet(var/datum/mind/player)
	if(!..())
		return
	to_chat(player.current, "The Emergency Response Team works for GRU Spetsnaz Division Delta; your team was formed to help the [station_name()] in their crisis. your task is to go and fix any problem and clean enemy presence.")
